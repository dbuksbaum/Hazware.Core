...
namespace Hazware.Logging
{
  ...
  ///<summary>
  /// Logger that outputs to <see cref="System.Diagnostics.Trace"/>.
  /// </summary>
  ///<typeparam name="T">The type associated with this logger.</typeparam>
  public sealed class TraceLogger<T> : AbstractLogger
  {
    #region Fields
    ...
    private void WriteLine(string level, string message, object[] args)
    {
      ...
      SysTrace.WriteLine(string.Format("[{0}] {1}", level, string.Format(message, args)), _logName);
    }
